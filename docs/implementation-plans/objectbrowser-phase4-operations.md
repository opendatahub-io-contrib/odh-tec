  ...
  try {
    ...
    await Promise.all(
      Array.from(selectedItems).map(path =>
        storageService.deleteFile(locationId, btoa(path))
      )
    );
...
const handleCreateDirectory = async (directoryName: string) => {
  ...
  try {
    ...
    Emitter.emit('notification', {
      ...
      description: error.response?.data?.message || `Failed to create directory "${directoryName}".`,
    });
  }
};
```
...
const handlePathClick = (newPath: string) => (event?: React.MouseEvent) => {
  ...
  navigate(newPath !== ''
    ? `/browse/${locationId}/${btoa(newPath)}`
    : `/browse/${locationId}`
  );
};
```
...
const handleLoadMore = () => {
  ...
  if (selectedLocation.type === 's3') {
    refreshFiles(selectedLocation, currentPath, paginationToken, true)
      .finally(() => setIsLoadingMore(false));
  } else {
    refreshFiles(selectedLocation, currentPath, null, true)
      .finally(() => setIsLoadingMore(false));
  }
};
```
